...
cp /etc/apache2/vhosts.d/openqa.conf.template \
   /etc/apache2/vhosts.d/openqa.conf
    \end{lstlisting}
\item enable apache modules and restart apache
    \begin{lstlisting}
a2enmod headers
a2enmod proxy
a2enmod proxy_http
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item first, switch off https by editing \texttt{/etc/openqa/openqa.ini}
    \begin{lstlisting}
[openid]
httpsonly = 0
    \end{lstlisting}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item the \texttt{fetchneedles} tool can be used to clone from github
    \lstset{style=mybash}
    \begin{lstlisting}
/usr/lib/os-autoinst/tools/fetchneedles --shallow
    \end{lstlisting}
  \end{itemize}
\end{frame}
